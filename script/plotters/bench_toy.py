...
class bench_toy(Plotter):
    def __init__(self, row, settings):
        super().__init__(row, settings, "bench_toy", weights=True)
    ...
    def _process_single_row(self, row):
        ...
        self.weights.setdefault(key, {}).setdefault(
            train_dataset, int(row["test-size"])
        )
        self.data.setdefault(key, {}).setdefault(model, {}).setdefault(
            train_dataset, {}
        ).setdefault(n_samples, 0)
        self.data[key][model][train_dataset][n_samples] = float(
            row["match_percentage"].replace("%", "")
        )
        ...
        for test_settings, datasets in self.weights.items():
            ...
            for dataset in datasets:
                if self.weights[test_settings][dataset] is None:
                    ...
                    continue
                self.weights[test_settings][dataset] = (
                    float(self.weights[test_settings][dataset]) / total
                )
        return self.weights
    ...
    def _extra(self):
        ...
        self._diz_to_csv(
            total_gain, os.path.join(self.dest_folder, "marginal_gain_absolute.csv")
        )
        self._diz_to_csv(
            relative_gain, os.path.join(self.dest_folder, "marginal_gain_relative.csv")
        )
    ...
    def _compute_marginal_gain(
        ...
        for model in gain_total:
            for i in range(len(self.x_data) - 1):
                ...
                gain_total[model][label] = (
                    round(sum(gt_values) / len(gt_values), 2) if gt_values else 0.0
                )
                gain_relative[model][label] = (
                    round(sum(gr_values) / len(gr_values), 2) if gr_values else 0.0
                )
