...
class Model:
    ...
    def __init__(self, s):
        ...
        self.mode = SampleMode.TOPK
        if not status and self.estimate_pwd is not None:
            self.mode = SampleMode.IID
            print(
    ...
    def _prepare_paths(self):
        ...
        self.path_to_samples_file_topk = os.path.join(
            self.path_to_results_dir, "samples.topk.gz"
        )
        self.path_to_samples_file_iid = os.path.join(
            self.path_to_results_dir, "samples.iid.gz"
        )
    ...
    def get_generator_for_sample_file(
        self, log_2: bool = False
        ...
        """
        path_to_file = (
            self.path_to_samples_file_topk
            if self.mode is SampleMode.TOPK
            else self.path_to_samples_file_iid
        )
        with gzip.open(path_to_file, "rb") as fd:
            while True:
    ...
    def error_range(self, target: float) -> float:
        return math.pow(2, target - 1) / math.sqrt(self.n_samples)
    ...
    def montecarlo_estimation(self) -> float:
        ...
        for _ in range(self.n_samples):
            _, ell = next(generator)
            if not ell:
                break
            if ell < target:
                my_sum += math.pow(2, ell)
    ...
    def start_estimation(self, checkpoint_name: str) -> float:
        ...
        file_to_load = self.path_to_samples_file_iid
        s = (
    ...
    def iid_evaluation(
        self, evaluation_batch_sizedation_mode=False, validation_mode=False
    ):
        print(f"Generating {self.n_samples} passwords...")

        evaluation_batch_size: int = int(self.params["eval"]["evaluation_batch_size"])
        n_batches: int = 0
        if self.n_samples < evaluation_batch_size:
            n_batches, evaluation_batch_size = 1, int(self.n_samples)
        else:
            n_batches = math.floor(self.n_samples / evaluation_batch_size)

        eval_dict: dict = self.eval_init(self.n_samples, evaluation_batch_size)

        progress_bar = tqdm(range(n_batches))
        progress_bar.set_description(desc="Generating sample batch")
        count: int = 0
        with gzip.open(self.path_to_samples_file_iid, "wt") as file:
            for _ in range(n_batches):
                ...
                for sample in generated_passwords:
                    # assert isinstance(sample,tuple) and len(sample) > 0 and isinstance(sample[0] ,str) and isinstance(sample[1] ,float)
                    file.write(f"{sample[0]} {sample[1]}\n")
                    count += 1

            progress_bar.set_postfix(
                {
                    "Sampled pwds %": ({(count / self.n_samples) * 100.0}),
                }
            )
