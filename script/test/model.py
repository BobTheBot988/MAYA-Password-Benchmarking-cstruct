...
from typing import Dict, Generator, Iterable, Literal
...
class Model:
    ...
    def fast_montecarlo_estimation(
        self, vals: Iterable[float], target_val: float
    ) -> float:
        """
        Monte Carlo rank estimator:
          R̂ = (1/n) * sum 2**ell_i * 1[ell_i < ell_target]
        where ell = -log2 p. 'vals' and 'target_val' can be either p or ell depending on self.log_2.
        """
        # Materialize exactly n samples if you want to enforce count:
        # arr = np.fromiter(vals, dtype=np.float64, count=self.n_samples)
        arr = np.fromiter(vals, dtype=np.float64)
        if arr.size == 0:
            return 0.0
        ...
        print(f"[I] - #hits={hits},hit_rate={hit_rate}")
        # Should satisfy: hit_rate ≈ R_hat / 2**ell_target
        return contrib.mean()
    ...
    def montecarlo_estimation(self) -> float:
        ...
        generator: Generator[float, None, None] = self.get_generator_for_sample_file()
        ...
        target = -np.log2(target) if self.log_2 else target
        my_sum: float = 0
                ...
                for b_ix in range(total_batches):
                    ...
                    if self.save_samples:
                        ...
                        for _pwd, _ell in generated:
                            _ell = float(_ell)
                            yield _ell if not self.log_2 else -np.log2(_ell)
                            count += 1
