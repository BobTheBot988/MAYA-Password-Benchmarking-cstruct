...
from typing import Any, Generator, Literal, List, Iterable, Tuple
...
class Model:
    ...
    def _parse_settings(self):
        ...
        self.estimate_pwd: str = str(self.settings.get("estimate_pwd"))
    ...
    def montecarlo_sampling(self, path, *args, **kwargs):
        raise NotImplementedError()

    def _read_from_mc_file(self, path) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        This function follows section 3.2 of the ccs15 paper
        Parameters:
        ...
        with gzip.open(path, "rt") as f:
            r = csv.reader(f)
            probs: Generator[float, None, None] = (float(row[1]) for row in r)

            A: torch.Tensor = torch.asarray(
                tuple(probs), device=self.device, dtype=torch.float64
            )
        # Sorting according to the specification
        ...
        # C is then updated with for i in range(len(A))  C[i] = C[i]/n
        C /= A.numel()
        return A, C

    def _write_mc_file(self, path: str, A: torch.Tensor, C: torch.Tensor):
    ...
    def _build_mc_curve(self, eval_dict) -> Tuple[torch.Tensor, torch.Tensor]:
        # Defining important paths
        mc_rank_curve_path = os.path.join(self.path_to_guesses_dir, "mc_rank_curve.gz")
        A_torch_path = os.path.join(self.path_to_guesses_dir, "A.pt")
        ...
        if not os.path.exists(mc_path) and not (
            ...
            # reading from the file and constructing the Tensors
            A, C = self._read_from_mc_file(mc_path)
            if not os.path.exists(mc_rank_curve_path):
                self._write_mc_file(mc_rank_curve_path, A, C)
...
def get_generator_from_topk_file(
    ...
    with gzip.open(topk_file_path, "rt") as f:
        while True:
            ...
            yield (
                pwd,
                torch.scalar_tensor(float(prob), device=device, dtype=torch.float64),
            )
