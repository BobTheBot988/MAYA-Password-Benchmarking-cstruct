...
class Model:
    ...
    def fast_montecarlo_estimation(
        self, ells: Iterable[float], ell_target: float
    ) -> float:
        ells = np.fromiter(ells, dtype=np.float64)
        return np.exp2(ells[ells < ell_target]).mean() if ells.size else 0.0
    ...
    def enumeration_of_pwd(self) -> int:
        ...
        generator: Iterable[float] = self.get_generator_for_sample_file(log_2=True)
        print("[I] - Getting the target probability")
        # The probability should be in the -log_2 form
        target: float = self.get_string_probability()
    ...
    def start_estimation(self, checkpoint_name: str) -> float:
        ...
        if not os.path.exists(file_to_load):
            gen = self.iid_evaluation(log_2=True)
        else:
            gen = self.get_generator_for_sample_file(log_2=(True))
        rank: float = self.fast_montecarlo_estimation(
            gen, self.get_string_probability(log_2=True)
        )
    ...
    def iid_evaluation(
        self, *, validation_mode: bool = False, log_2=False
    ) -> Iterable[float]:
        """
                ...
                for b_ix in range(total_batches):
                    ...
                    if self.save_samples:
                        ...
                        for _pwd, _ell in generated:
                            _ell = float(_ell)
                            yield _ell if not log_2 else -np.log2(_ell)
                            count += 1
