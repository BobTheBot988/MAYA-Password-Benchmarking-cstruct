...
class FLA(Model):
    ...
    def generate_heap(
        self, temp_file_name: str, eval_dict: Dict[str, Model.T], n_gen: int
    ) -> heapcy.Heap:
        with open(temp_file_name, "rb") as f_open:
            ...
            progress_bar = tqdm(range(n_gen))
            progress_bar.set_description(desc="Writing to heap")
            while True:
                ...
                progress_bar.set_postfix(
                    {
                        "heap_items %": {len(min_heap_n_most_prob) / 100},
                    }
                )
    ...
    def sample(
        ...
        min_heap_n_most_prob: heapcy.Heap = self.generate_heap(
            temp_file_name, eval_dict, n_gen
        )
