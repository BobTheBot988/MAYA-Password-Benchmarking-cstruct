...
class FLA(Model):
    ...
    def guesser_build(self, eval_dict: Dict[str, Any]) -> Guesser:
        return Guesser(
            model=self.model,
            params=self.params,
            data=self.data,
    ...
    def montecarlo_sampling(self, path, eval_dict: Dict):
        guess: Guesser = self.guesser_build(eval_dict)
        guess.one_batch_to_control_them_all(self.n_samples, True, path)
