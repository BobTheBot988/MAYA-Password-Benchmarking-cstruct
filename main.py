...
def parse_args():
    ...
    general.add_argument(
        ...
        type=int,
        choices=[0, 1],
        help="0 = suppress stderr to file, 1 = show in console.",
    )
    general.add_argument(
        "--autoload",
        type=int,
    ...
    general.add_argument(
        ...
        type=int,
        choices=[0, 1],
        help="0 = skip test if already done, 1 = overwrite previous results",
    )
    general.add_argument(
        "--path_to_checkpoint", type=str, help="Path to model checkpoint file."
    )
    general.add_argument(
        "--n_samples", nargs="+", type=int, help="Number(s) of passwords to generate."
    )
    general.add_argument(
        "--estimate_pwd",
        nargs="+",
        type=str,
        help="""Estimate the password(s) rank with montecarlo requires the model to have generatedthe sample:
        \n 10**3<=sample_size<=10**5
        \n if not found then it will call the sampling by it self""",
    )
    general.add_argument(
        "--test_config", type=str, help="Path to YAML scenario configuration file."
    )
    general.add_argument(
        "--guesses_file",
        nargs="+",
        type=str,
        help="Use existing password guesses from file(s).",
    )
    general.add_argument(
        "--sub_samples_from_file",
        nargs="+",
        type=str,
        help="Subsample passwords from given file(s).",
    )
    general.add_argument(
        "--data_to_embed",
        nargs="+",
        type=str,
        help="Path(s) to data for PassFlow encoder embedding.",
    )
    general.add_argument(
        "--save_guesses",
        type=int,
        choices=[0, 1],
        default=1,
        help="1 = save guesses, 0 = don't save guesses",
    )
    general.add_argument(
        "--save_matches",
        type=int,
    ...
    pre_split.add_argument(
        "--char_bag", nargs="+", type=str, help="Character set(s) to consider."
    )
    pre_split.add_argument(
        "--train_datasets", nargs="+", type=str, help="Name(s) of training dataset(s)."
    )
    ...
    split.add_argument(
        ...
        nargs="+",
        type=int,
        help="Percentage split for training data.",
    )

    # Post-split
    post_split.add_argument(
        "--train_chunk_percentage",
        nargs="+",
        type=int,
        help="Subset of training data to use.",
    )

    # Test
    test.add_argument(
        "--test_datasets", nargs="+", type=str, help="Name(s) of testing dataset(s)."
    )
    test.add_argument(
        "--test_frequency",
        nargs="+",
        type=int,
        help="Top/bottom N% of test set by frequency.",
    )
